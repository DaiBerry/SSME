...
class MVTEC(data.Dataset):
    ...
    def __init__(self, indexes, normal_class,
        ...
        print("Datasets are on {}".format(folder))
        ...
        if self.task == 'train':
            ...
            for ind in self.indexes:
                im = Image.open(path2 + images[ind]).convert("RGB")
                # 将图像拆分为三个通道
                r, g, b = im.split()

                # 将每个通道调整为目标大小
                im2 = r.resize((128, 128))
                im3 = g.resize((128, 128))
                im4 = b.resize((128, 128))
            ...
            for ty in types:
                ...
                for image in images:
                    im = Image.open(path2 + ty + '/' + image).convert("RGB")
