...
_C.TRAIN.CHECKPOINT_FILE_PATH = "MViTv2_B_in21k.pyth"

# If True, reset epochs when loading checkpoint.
_C.TRAIN.CHECKPOINT_EPOCH_RESET = True

# If True, use FP16 for activations 半精度浮点数;较少内存
...
# Cutmix alpha.较大的 alpha 值会导致目标值更多地混合在一起。
_C.MIXUP.CUTMIX_ALPHA = 1.0

# Probability of performing mixup or cutmix when either/both is enabled.1代表始终进行mixup操作而进行cutmix的概率为0
_C.MIXUP.PROB = 1.0
...
# Activation checkpointing enabled or not to save GPU memory.如果 GPU 内存有限，可以将该参数设置为 True，以减少内存占用
...
# Base embedding dimension for the transformer.def = 96
_C.MVIT.EMBED_DIM = 128

# Base num of heads for the transformer. def=1
_C.MVIT.NUM_HEADS = 8
